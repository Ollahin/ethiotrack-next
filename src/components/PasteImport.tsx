...
import { addTransactionsBulk, updateTransaction, useAgents, useTransactions } from "@/lib/db";
...
export function PasteImport() {
  ...
  async function importAll() {
    ...
    const inputs: Array<Omit<Transaction, "id" | "createdAt">> = ok.map(({ row, agent }) => ({
      type: row.type!,
              ...
              {row.ok ? (
                <div className="space-y-1">
                  <div className="flex items-center justify-between gap-2 flex-wrap">
                    ...
                    <span className="text-xs uppercase font-semibold text-ink-soft">{row.channel}</span>
                  </div>
