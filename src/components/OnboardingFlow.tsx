...
export function OnboardingFlow() {
  ...
  const handleNext = () => {
    const steps: Step[] = ["welcome", "master-pin", "user-profile", "banks", "distributors", "agents", "finish"];
        ...
        {step === "welcome" && (
          <div className="space-y-6 text-center">
            ...
            <div className="space-y-2">
              ...
              <p className="text-white/60">Let's get your local ledger set up. Your data stays completely on this device.</p>
            </div>
          ...
          <div className="space-y-6">
            <div className="space-y-2">
              <div className="flex items-center gap-2 text-primary">
                ...
                <span className="text-sm font-semibold uppercase tracking-wider">Step 1: Ownership</span>
              </div>
              <h2 className="text-2xl font-bold">Set Master PIN</h2>
              <p className="text-sm text-white/60">This PIN is for the owner. You'll need it once a month to keep the app active.</p>
            </div>
            ...
            <div className="flex gap-3">
              ...
              <Button onClick={setupSecurity} disabled={busy || masterPin.length < 6} className="flex-[2]">Continue</Button>
            </div>
          </div>
        )}
          ...
          <div className="space-y-6">
            <div className="space-y-2">
              <div className="flex items-center gap-2 text-primary">
                ...
                <span className="text-sm font-semibold uppercase tracking-wider">Step 2: Operator</span>
              </div>
            ...
            <div className="flex gap-3">
              <Button variant="ghost" onClick={handleBack} className="flex-1">Back</Button>
              <Button onClick={setupUser} disabled={busy || userPin.length < 6 || !userName} className="flex-[2]">Continue</Button>
            </div>
          </div>
        )}
        ...
        {step === "banks" && (
          <div className="space-y-6">
            <div className="space-y-2">
              ...
              <p className="text-sm text-white/60">Add at least one bank account or wallet to track receipts.</p>
            </div>
            
            <div className="space-y-4 p-4 rounded-xl bg-white/5 border border-white/10">
              <div className="space-y-2">
                <Label>Display Name</Label>
                <Input value={bankName} onChange={e => setBankName(e.target.value)} placeholder="Main CBE" className="bg-ink border-white/10" />
              </div>
              <div className="grid grid-cols-2 gap-3">
                ...
                <div className="space-y-2">
                  ...
                  <Input value={bankAcct} onChange={e => setBankAcct(e.target.value)} placeholder="1234" className="bg-ink border-white/10" />
                </div>
              </div>
            ...
            <div className="flex gap-3">
              <Button variant="ghost" onClick={handleBack} className="flex-1">Back</Button>
              <Button onClick={handleNext} disabled={banksAdded === 0} className="flex-[2]">Continue</Button>
            </div>
            {banksAdded === 0 && <p className="text-[10px] text-white/40 text-center">Add at least one bank to continue.</p>}
          </div>
        )}
          ...
          <div className="space-y-6">
            <div className="space-y-2">
              <div className="flex items-center gap-2 text-primary">
                ...
                <span className="text-sm font-semibold uppercase tracking-wider">Step 4: Suppliers</span>
              </div>
            ...
            <div className="space-y-4 p-4 rounded-xl bg-white/5 border border-white/10">
              <div className="space-y-2">
                ...
                <Input value={distName} onChange={e => setDistName(e.target.value)} placeholder="MJ Telecom" className="bg-ink border-white/10" />
              </div>
              <div className="space-y-2">
                <Label>Statement Format</Label>
                <Select value={distFormat} onValueChange={v => setDistFormat(v as DistributorStatementFormat)}>
                  <SelectTrigger className="bg-ink border-white/10">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
                    {Object.keys(DISTRIBUTOR_FORMAT_LABEL).map(k => (
                      <SelectItem key={k} value={k}>{DISTRIBUTOR_FORMAT_LABEL[k as DistributorStatementFormat]}</SelectItem>
                    ))}
                  </SelectContent>
                </Select>
              </div>
        ...
        {step === "agents" && (
          <div className="space-y-6">
            <div className="space-y-2">
              <div className="flex items-center gap-2 text-primary">
                <Users className="h-5 w-5" />
                <span className="text-sm font-semibold uppercase tracking-wider">Step 5: Customers</span>
              </div>
            ...
            <div className="space-y-4 p-4 rounded-xl bg-white/5 border border-white/10">
              <div className="space-y-2">
                ...
                <Input value={agentName} onChange={e => setAgentName(e.target.value)} placeholder="Alexo" className="bg-ink border-white/10" />
              </div>
              <div className="space-y-2">
                <Label>Phone (Optional)</Label>
                <Input value={agentPhone} onChange={e => setAgentPhone(e.target.value)} placeholder="09..." className="bg-ink border-white/10" />
              </div>
        ...
        {step === "finish" && (
          <div className="space-y-6 text-center">
            ...
            <div className="space-y-2">
              ...
              <p className="text-white/60">EthioTrack is configured and ready. Your local database is active.</p>
            </div>
