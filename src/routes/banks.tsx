    ...
    <div className="max-w-3xl mx-auto p-4 md:p-6 space-y-4">
      <div>
        ...
        <p className="text-sm text-ink-soft">
          Your own accounts and wallets used to receive/pay money.
        </p>
      </div>
      <div className="rounded-xl border border-border bg-card p-4 shadow-sm space-y-3">
        <div className="grid grid-cols-2 gap-3">
          <div>
            <Label>Name</Label>
            <Input value={name} onChange={(e) => setName(e.target.value)} placeholder="CBE main" />
          </div>
          <div>
            <Label>Account #</Label>
        ...
        <Button
          onClick={async () => {
            ...
            await upsertBank({
              ...
              accountNumber: acct,
              channel,
              openingBalanceSantim: parseEtbToSantim(opening) ?? 0,
            });
            setName("");
            setAcct("");
            setOpening("");
          ...
          <li key={b.id} className="p-3 flex items-center justify-between">
            <div>
              ...
              <div className="text-xs text-ink-soft">
                {b.channel}
                {b.accountNumber ? ` · ${b.accountNumber}` : ""} · opening{" "}
                {formatEtb(b.openingBalanceSantim)}
              </div>
            </div>
            <Button
              variant="ghost"
